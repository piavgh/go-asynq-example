...
func main() {
	...
	for {
		...
		// Define tasks.
		t1, err := tasks.NewWelcomeEmailTask(userID)
		if err != nil {
			log.Fatal(err)
		}

		t2, err := tasks.NewReminderEmailTask(userID, time.Now().Add(delay))
		if err != nil {
			log.Fatal(err)
		}
		...
		fmt.Printf("Send task to queue with id: %v\n", userID)
